...
public class GraphDBInterface {
	...
	private static String GRAPHDB_SERVER = Configuration.SPARQL_ENDPOINT;
	private static String REPOSITORY_ID = Configuration.GRAPH_DB_NAME;
	...
	private void executeStatement(RepositoryConnection repositoryConnection, String insertStatement) {
		Update updateOperation = repositoryConnection.prepareUpdate(QueryLanguage.SPARQL, insertStatement);
	...
	static {
		...
		strInsertThing = "PREFIX btea: <http://www.bt.schema.siemens.io/custom/btea#>\n"
		...
		strDeleteEvent = "DELETE WHERE {" + "<%id> ?p ?o ." + "}";

		strQuery = "SELECT ?thing FROM DEFAULT WHERE {" + "?thing ?p <https://www.w3.org/2019/wot/td#Thing> .}";
	}
}
